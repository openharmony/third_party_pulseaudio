...
int pa_asyncmsgq_send(pa_asyncmsgq *a, pa_msgobject *object, int code, const void *userdata, int64_t offset, const pa_memchunk *chunk) {
    ...
    if (executionTime > OH_DAEMON_TIMEOUT_THRESHOLD_ON_US) { // too long block of daemon thread, dangerous
        AUDIO_WARNING_LOG("Execution time of this msg is too long: qLen[%{public}u], MSG[%{public}d] (%{public}lums)",
            PaAsyncqGetNumToRead(a->asyncq), code, (uint64_t)executionTime / PA_USEC_PER_MSEC);
    }
    return i.ret;
}
